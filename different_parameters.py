...
def parametric_study(a1, a2, a3, eta_f_guess, f_start = 1e-4):
    ...
    eta_f, (sol_eta, sol_x ) = solver.find_etaf(eta_f_guess[0], f_start=f_start)
    res = solver._check_integral_condition(sol_eta, sol_x)
    return (eta_f, (sol_eta, sol_x ), res), (eta_data, x_data, res_data)
...
if __name__ == '__main__':
    ...
    for params in param_sets:
        etaf, (eta, x) = driver(*params, etaf_guess=etaf_guess, f_start = fstart)
        ax[0].plot(eta, x[0])
        ax[1].plot(eta, x[2], label = "$\\alpha_1 = {}, \\alpha_2 = {}, \\alpha_3 = {}$".format(*params))
        ax[0].text(0, x[0][0], "$f(0) = {:.4f}$".format(x[0][0]))
        ax[0].text(eta[-1], fstart, "$\\eta_f = {:.4f}$".format(etaf))
